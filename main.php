...
<?php
  ...
  try {
    ...
    if(isset($_POST['submit']) && !empty($_POST['post'])) {
      ...
      $prepare = $dbh->prepare("INSERT INTO text_posts (id, user_id, text, date) VALUES (NULL, :user_id, :text_post, CURRENT_TIMESTAMP)");
      $prepare->bindValue(':user_id', (int)$user_id, PDO::PARAM_INT);
      $prepare->bindValue(':text_post', $text_post, PDO::PARAM_STR);
      $prepare->execute();
    }
    // $prepare = $dbh->prepare('SELECT * FROM users, text_posts WHERE users.id = text_posts.user_id AND users.id = ?');
